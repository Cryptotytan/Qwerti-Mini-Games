...
// ─────────────────────────────────────────────
//  IN-MEMORY LEADERBOARD (no localStorage)
// ─────────────────────────────────────────────
// We store leaderboard in a module-level variable so it persists across re-renders
// but resets on page refresh (required since localStorage is unavailable in this env)
let _lbStore = { easy:[], normal:[], hard:[] };

function loadLB() {
  return JSON.parse(JSON.stringify(_lbStore)); // deep clone
}

function saveLB(d) {
  _lbStore = JSON.parse(JSON.stringify(d));
}
...
export default function App() {
  ...
  // Use a ref to always have the latest secs value inside the interval/handlers
  ...
  // Keep secsRef in sync with secs state
  ...
  function connectHandle() {
    ...
    if (!h || h.replace("@","").length < 2) {
      setHErr("Please enter a valid handle."); return;
    }
    setHErr("");
    setHandle(h);
    setScreen("levels");
  ...
  // Uses a ref for tiles to avoid stale closure issues
  ...
  function handleCheck() {
    ...
    const correctSet = [];
    let totalCorrect = 0;
    const totalTiles = currentTiles.length;

    for (let posIdx = 0; posIdx < currentTiles.length; posIdx++) {
      if (currentTiles[posIdx] === posIdx) {
        correctSet.push(posIdx);
        totalCorrect++;
      }
    }
    ...
    if (totalCorrect === totalTiles) {
      // ── PUZZLE COMPLETE ──
      stopClock();
      const finalTime = secsRef.current;
      setWon(true);
  ...
  function onDrop(e, posIdx) {
    ...
    setTiles(prev => {
      ...
      tilesRef.current = next; // keep ref in sync immediately
      return next;
    });
    setDrag(null);
    setOver(null);
  ...
  return (
    <div style={{fontFamily:"monospace",background:C.bg,minHeight:"100vh",color:C.text,overflowX:"hidden",position:"relative"}}
      ...
      <style>{`
        ...
        @keyframes mgpulse{0%,100%{box-shadow:0 0 0 0 rgba(217,70,239,0.4)}50%{box-shadow:0 0 0 8px rgba(217,70,239,0)}}
      ...
      {screen==="connect" && (
        <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:28,padding:24,position:"relative",zIndex:1,animation:"fadeUp 0.5s ease"}}>
          ...
          {/* ── Handle card – all magenta/purple theme, no blue ── */}
          <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:16,padding:32,width:"100%",maxWidth:420,display:"flex",flexDirection:"column",gap:20}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              {/* Icon uses magenta theme instead of blue */}
              <div style={{width:44,height:44,borderRadius:"50%",background:C.mdim,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.3rem",animation:"mgpulse 2.5s ease-in-out infinite"}}>𝕏</div>
              <div>
            ...
            <div>
              ...
              <div style={{position:"relative",marginTop:8}}>
                {/* @ sign uses magenta */}
                <span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",color:C.mg,fontWeight:800,fontSize:"1rem",pointerEvents:"none"}}>@</span>
                <input value={hInput.startsWith("@")?hInput.slice(1):hInput}
                  ...
                  style={{display:"block",width:"100%",background:C.bg,
                    border:`1px solid ${hInput?C.mg:C.border}`,
                    borderRadius:10,padding:"12px 14px 12px 36px",color:C.text,fontFamily:"monospace",fontSize:"1rem",fontWeight:700,
                    boxShadow:hInput?`0 0 0 3px ${C.mdim}`:"none",
                    transition:"all 0.2s"}}/>
              </div>
            </div>
            {hErr && <div style={{fontSize:"0.72rem",color:C.red,textAlign:"center"}}>{hErr}</div>}
            {/* Connect button uses magenta gradient instead of blue */}
            <button onClick={connectHandle}
              style={{width:"100%",background:`linear-gradient(135deg,${C.mg},#a21caf)`,color:"#fff",border:"none",borderRadius:10,padding:"13px 22px",fontFamily:"monospace",fontWeight:800,fontSize:"0.95rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:`0 4px 20px ${C.mglow}`,transition:"all 0.15s"}}
      ...
      {screen==="levels" && (
        <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:28,padding:24,position:"relative",zIndex:1,animation:"fadeUp 0.4s ease"}}>
          <div style={{width:"100%",maxWidth:700,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            {/* ✅ FIXED: Back button now correctly navigates to "connect" screen */}
            <GBtn onClick={()=>setScreen("connect")}>← Change Handle</GBtn>
            <div style={{display:"flex",alignItems:"center",gap:8,background:C.mdim,border:`1px solid ${C.border}`,borderRadius:20,padding:"6px 14px"}}>
              <span style={{color:C.mg}}>𝕏</span>
          ...
          {/* ✅ FIXED: AppBtn renders correctly here */}
          <AppBtn/>
        </div>
      )}
        ...
        <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",padding:"16px 14px",gap:14,position:"relative",zIndex:1}}>
          ...
          <div style={{width:"100%",maxWidth:520,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              {/* ✅ FIXED: Back button stops timer and navigates to levels */}
          ...
          <div style={{display:"flex",gap:10,width:"100%",maxWidth:520}}>
            {/*
              ✅ FIXED: handleCheck reads from tilesRef (always current),
              and secsRef (always current) — no stale closure issues.
            */}
            <PBtn onClick={handleCheck} style={{flex:1}}>✓ Check Placement</PBtn>
            {/*
              ✅ FIXED: handleShuffle always produces a new scrambled array.
              Timer keeps running.
            */}
      ...
      {screen==="leaderboard" && (
        <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",padding:24,gap:20,position:"relative",zIndex:1,animation:"fadeUp 0.4s ease"}}>
          <div style={{width:"100%",maxWidth:640,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
            ...
            {/* ✅ FIXED: Back button goes to levels */}
                      ...
                      <tr key={i} style={{borderBottom:"1px solid rgba(255,255,255,0.03)",background:isMe?"rgba(217,70,239,0.06)":"transparent"}}>
                        ...
                        <td style={{padding:"12px 16px"}}>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            <span style={{color:C.mg,fontSize:"0.8rem"}}>𝕏</span>

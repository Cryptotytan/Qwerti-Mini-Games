...
// ── Universal Leaderboard API ─────────────────────────────
var API_BASE = "/api/leaderboard";

// Fetch leaderboard for a given level from the server
...
// Submit score to server, returns rank info
...
// ── Local best times cache (per session) ──────────────────
...
function doCheck() {
  ...
  if (correct.length === tiles.length) {
    ...
    // Submit to universal leaderboard
    submitScore(currentLevelKey, handle, finalTime, function(err, data) {
      if (err || !data) {
        rMsg.textContent = "Score saved locally. Leaderboard unavailable.";
        rMsg.style.color = "var(--dim)";
        return;
      }
      if (data.inTop10 && data.rank) {
        rMsg.textContent = "🏆 You ranked #"+data.rank+" on the global "+currentLevelKey+" leaderboard!";
        rMsg.style.color = "var(--mg)";
      } else {
        rMsg.textContent = "Finish faster to crack the global top "+LB_MAX+"!";
...
function showLeaderboard() {
  ...
  fetchLeaderboard("easy", function(err, entries) { renderLb(entries, "easy"); });
}
...
function setLbTab(lvl, btn) {
  ...
  fetchLeaderboard(lvl, function(err, entries) { renderLb(entries, lvl); });
}

function renderLb(entries, lvl) {
